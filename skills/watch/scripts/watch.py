...
def main() -> int:
    ...
    if end_sec is not None and end_sec <= (start_sec if start_sec is not None else 0.0):
        raise SystemExit("--end must be greater than --start")
    if full_duration > 0 and start_sec is not None and start_sec >= full_duration:
        raise SystemExit(f"--start {start_sec:.1f}s is past end of video ({full_duration:.1f}s)")
    if full_duration > 0 and end_sec is not None and end_sec > full_duration:
        print(
            f"[watch] --end {end_sec:.1f}s is past end of video "
            f"({full_duration:.1f}s); clamping to video end",
            file=sys.stderr,
        )
