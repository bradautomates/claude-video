...
| `3` | No Whisper backend (no local `whisper-cli` AND no API key) | Run installer to scaffold `.env`, then ask user for a key (or install `whisper-cpp`) |
...
**If no Whisper backend is configured after install:** use `AskUserQuestion` to offer three options — (1) install `whisper-cpp` locally (`brew install whisper-cpp` on macOS — free, Metal-accelerated, no network), (2) a Groq API key (cheaper, faster than OpenAI), or (3) an OpenAI key. For options 2/3, write into `~/.config/watch/.env` — set the matching `GROQ_API_KEY=...` or `OPENAI_API_KEY=...` line. If they don't want to set up Whisper at all, proceed with `--no-whisper` and tell them videos without native captions will come back frames-only.
...
- `--whisper local|groq|openai` — force a specific Whisper backend (default priority: local whisper.cpp → Groq → OpenAI, picking the first that's available)
...
2. **Whisper fallback.** If no captions came back (or the source is a local file), the script extracts audio (`ffmpeg -vn -ac 1 -ar 16000 -b:a 64k`, ~0.5 MB/min) and runs it through whichever Whisper backend is available, in priority order:
   - **Local** — `whisper-cli` (whisper.cpp) with a `ggml-*.bin` model. Free, Metal-accelerated on macOS, no network. Install on macOS with `brew install whisper-cpp` (ships `ggml-large-v3-turbo.bin` automatically). Auto-detected — no env var needed.
   - **Groq** — `whisper-large-v3`. Cloud fallback: cheaper and faster than OpenAI. Get a key at console.groq.com/keys.
   - **OpenAI** — `whisper-1`. Cloud fallback. Get a key at platform.openai.com/api-keys.

API keys live in `~/.config/watch/.env`. The script picks the first available backend in the priority order above. Override with `--whisper local|groq|openai`. Use `--no-whisper` to skip the fallback entirely.
...
- **Whisper request fails** → the error is printed to stderr (cloud: invalid key, rate limit, or 25 MB upload limit on a very long video; local: missing model file or `whisper-cli` crash). The report will say "none available" for transcript. You can retry with a different backend via `--whisper local|groq|openai`.
...
- Runs `whisper-cli` (whisper.cpp) locally on the extracted audio when a `ggml-*.bin` model is present (auto-detected, preferred — no network) and Groq/OpenAI aren't forced
- Sends the extracted audio clip to Groq's Whisper API (`api.groq.com/openai/v1/audio/transcriptions`) when `GROQ_API_KEY` is set and local Whisper isn't available (or when `--whisper groq` is forced)
- Sends the extracted audio clip to OpenAI's audio transcription API (`api.openai.com/v1/audio/transcriptions`) when `OPENAI_API_KEY` is set and neither local nor Groq is available (or when `--whisper openai` is forced)
...
**Bundled scripts:** `scripts/watch.py` (entry point), `scripts/download.py` (yt-dlp wrapper), `scripts/frames.py` (ffmpeg frame extraction), `scripts/transcribe.py` (caption selection + Whisper orchestration), `scripts/whisper.py` (local whisper.cpp + Groq / OpenAI clients), `scripts/setup.py` (preflight + installer)
