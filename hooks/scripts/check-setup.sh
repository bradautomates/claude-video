...
HAS_MUAPI="$(read_key MUAPI_API_KEY)"
if [[ -z "$HAS_MUAPI" ]]; then
  HAS_MUAPI="$(read_key MU_API_KEY)"
fi
...
elif [[ -z "$HAS_GROQ" && -z "$HAS_OPENAI" && -z "$HAS_MUAPI" ]]; then
  echo "/watch: ready for videos with native captions. Add GROQ_API_KEY (preferred), OPENAI_API_KEY, or MUAPI_API_KEY to ~/.config/watch/.env to unlock Whisper fallback."
else
  echo "/watch: ready."
fi
