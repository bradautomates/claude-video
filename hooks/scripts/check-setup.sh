...
HAS_ATLAS="$(read_key ATLASCLOUD_API_KEY)"
...
elif [[ -z "$HAS_GROQ" && -z "$HAS_OPENAI" && -z "$HAS_ATLAS" ]]; then
  echo "/watch: ready for videos with native captions. Add GROQ_API_KEY (preferred), OPENAI_API_KEY, or ATLASCLOUD_API_KEY to ~/.config/watch/.env to unlock transcription fallback."
else
  echo "/watch: ready."
fi
