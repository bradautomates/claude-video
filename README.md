...
Zero config to start — `yt-dlp` and `ffmpeg` install on first run via `brew` on macOS (Linux/Windows print exact commands). Captions cover most public videos for free. For the occasional video with no captions, transcribe with a Whisper API key **or** fully offline with local whisper.cpp — no key, no upload.
...
4. **The transcript comes from one of three places.** First try: `yt-dlp` pulls native captions (manual or auto-generated) from the source. Free, instant, accurate-ish. Cloud fallback: extract a mono 16 kHz 64 kbps mp3 audio clip (~480 kB/min) and ship it to Whisper — Groq's `whisper-large-v3` (preferred — cheaper and faster) or OpenAI's `whisper-1`. Offline fallback: with no cloud key set, transcribe locally via whisper.cpp (`--whisper local`) — the audio never leaves the machine.
...
On the first `/watch` call, the skill runs `scripts/setup.py --check`. If `ffmpeg` / `yt-dlp` aren't on your PATH, or no transcription backend is configured, it walks you through fixing it:
...
- **Transcription** — scaffolds `~/.config/watch/.env` (mode `0600`) with placeholders for cloud keys and local whisper.cpp overrides.
...
| Whisper backup (offline) | `whisper-cpp` + a ggml model in `~/.config/watch/models/` | Free, no upload — audio stays local |
| Disable Whisper entirely | `--no-whisper` | Free, frames-only when no captions |

Local whisper.cpp is the **backup** for the cloud path. It needs **no key** (so no cloud transcription charge), and it kicks in two ways: automatically when no cloud key is set, **and as the failover if a configured cloud key fails at runtime** (bad key, rate limit, network) — so a failed key still returns a transcript instead of frames-only. Install whisper.cpp (`brew install whisper-cpp`), drop a model in place, or force it with `--whisper local`:
```bash
mkdir -p ~/.config/watch/models
curl -L -o ~/.config/watch/models/ggml-base.en.bin \
  https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-base.en.bin
```

For safety, all watch settings — cloud API keys and the `WHISPER_CPP_BIN` / `WHISPER_CPP_MODEL` overrides — are read only from the shell environment or `~/.config/watch/.env`, never from a project-local `.env`. That stops an untrusted repository from choosing which binary `/watch` runs, or silently redirecting your audio to a cloud key it planted.
...
- `--whisper groq|openai|local` — force a specific Whisper backend. `local` runs whisper.cpp offline (no key, no upload); auto-selection prefers a cloud key, then falls back to local.
...
│       ├── whisper.py            # Groq / OpenAI clients + offline whisper.cpp backend (pure stdlib)
...
Built on `yt-dlp`, `ffmpeg`, and Claude's multimodal `Read` tool. Whisper transcription via [Groq](https://groq.com), [OpenAI](https://openai.com), or offline [whisper.cpp](https://github.com/ggerganov/whisper.cpp).
